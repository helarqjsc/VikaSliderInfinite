function VikaSliderInfinite(obj){
  ...
  instance.move_carousel_end = obj['move_carousel_end'] || function(){};
	...
	instance.queue = function() {
		if (instance.queue_n > 0) {
			if (instance.queue_n>=1) { instance.speed = 300; }
			if (instance.queue_n>=2) { instance.speed = 200; }
			if (instance.queue_n>=3) { instance.speed = 100; }
			if (instance.queue_n>=4) { instance.speed = 50; }
			instance.queue_n = instance.queue_n - 1;
			if (instance.queue_n == 0) { instance.speed = 300; }
	...
	if (instance.animate == false) {
		...
		$("body").on("click", '#carousel'+instance.id+' .item', function(){
			...
			if (instance.animate == false) {
				...
				if (k <= instance.click_left) { return false; }
				if (k < 0) {
					k = k*-1;
					instance.queue_n = k;
					instance.direction = 'prev';
		...
		$('#carousel'+instance.id+' #slides'+instance.id+' ul').css({'left' : instance.left_value});
  $('.buttons .prev').click(function() {
    	...
    	if (instance.animate == false) {
    		...
    		if (instance.pos < 1) { instance.pos = $('#carousel'+i+' .item').length; }
    		instance.real_pos = instance.real_pos - 1;
    		if (instance.real_pos < 1) { instance.real_pos = $('#carousel'+i+' .item').length / division; }
    		...
    		$('#carousel'+instance.id+' #slides'+instance.id+' ul').animate({'left' : left_indent}, instance.speed,function(){
    ...
    $('.buttons .next').click(function() {
    	...
    	if (instance.animate == false) {
    		...
    		left_indent = parseInt($('#carousel'+i+' #slides'+i+' ul').css('left')) - instance.item_width;
    		instance.pos = instance.pos + 1;
    		if (instance.pos > $('#carousel'+i+' .item').length ) { instance.pos = 1; }
    		instance.real_pos = instance.real_pos + 1;
    		if (instance.real_pos > $('#carousel'+i+' .item').length / division) { instance.real_pos = 1; }
			  instance.move_carousel_start(instance, instance.real_pos);
    		if (instance.queue_n == 0) {
    		...
    		$('#carousel'+i+' #slides'+i+' ul').animate({'left' : left_indent}, instance.speed, function () {
    			i = $(this).parent().parent().data('carousel');
